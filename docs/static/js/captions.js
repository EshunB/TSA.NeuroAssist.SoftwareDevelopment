(function () {
  ...
  function renderSegments() {
    ...
    if (segments.length === 0) {
      container.innerHTML =
        '<p class="small muted">Press <strong>Space</strong> or <strong>Listen</strong>.</p>';
      return;
    }
    var html = "";
  ...
  function updateButtons(isListening) {
    ...
    if (btn) {
      btn.textContent = isListening ? "Pause" : "Listen";
  ...
  function handleSave() {
    ...
    if (!window.NAStorage) {
      if (errEl) {
        ...
        errEl.textContent = "Storage is not available in this page.";
      }
      finishSave();
      return;
    }
    ...
    window.NAStorage
      ...
      .catch(function (e) {
        if (errEl) {
          errEl.style.display = "block";
          errEl.textContent =
            (e && e.message) || "Failed to save transcript in the browser.";
        }
      })
      .finally(finishSave);
  }
